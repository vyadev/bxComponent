...
\Bitrix\Main\EventManager::getInstance()->addEventHandler("sale", "\Bitrix\Sale\Internals\Basket::OnAfterAdd", 'increaseCartCounter');
...
\Bitrix\Main\EventManager::getInstance()->addEventHandler("sale", "OnBeforeSaleBasketItemEntityDeleted", "decreaseCartCounter");
function decreaseCartCounter(\Bitrix\Main\Event $event)
{
  \Bitrix\Main\Loader::includeModule('iblock');
  $catalogIblockId = 2; // IBLOCK_ID каталога
  ...
  $res = \Bitrix\Iblock\ElementTable::getList(array(
    'order' => array('SORT' => 'ASC'),
    'select' => array('ID'),
    'filter' => array('NAME' => $name, 'IBLOCK_ID' => $catalogIblockId),
  ));
